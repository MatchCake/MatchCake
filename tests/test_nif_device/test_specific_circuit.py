...
def specific_matchgate_circuit(params_wires_list, initial_state=None, **kwargs):
    ...
    use_h_for_transition_matrix = kwargs.pop(
        "use_h_for_transition_matrix",
        MatchgateOperation.DEFAULT_USE_H_FOR_TRANSITION_MATRIX,
    )
    ...
    for params, wires in params_wires_list:
        ...
        MatchgateOperation(
            mg_params,
            wires=wires,
            use_h_for_transition_matrix=use_h_for_transition_matrix,
        )
...
@pytest.mark.parametrize(
    ...
    [
        (initial_binary_string, params_wires_list, contraction_strategy)
        for [initial_binary_string, params_wires_list], contraction_strategy in list(
            itertools.product(
                *[
                    __states_and_gates,
                    [None, "neighbours", "forward", "horizontal", "vertical"],
                ]
            )
        )
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "000000",
            [(mps.fSWAP, [0, 1]), (mps.HellParams, [2, 3]), (mps.fSWAP, [4, 5])],
        ),
        ("0000", [(mps.fSWAP, [0, 1]), (mps.fSWAP, [1, 2]), (mps.HellParams, [2, 3])]),
    ],
)
