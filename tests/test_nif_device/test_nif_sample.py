...
def test_qubit_by_qubit_sampling_with_probs_op_gen(operations_generator, num_wires):
    ...
    unique_states_probability = np.stack(
        [
            np.sum(np.isclose(nif_samples, state).all(axis=-1), axis=0)
            / nif_samples.shape[0]
            for state in unique_states
        ],
        axis=0,
    )
    unique_states_probability = unique_states_probability / np.sum(
        unique_states_probability, axis=0, keepdims=True
    )
    unique_states_expval = nif_device.get_states_probability(
        unique_states, np.arange(num_wires)
    )
    states_expval = unique_states_expval / np.sum(
        unique_states_expval, axis=0, keepdims=True
    )
