...
from matchcake.operations import SptmfRxRx, SptmFSwap, SptmRzRz, SptmIdentity, SptmFHH, SptmRyRy, \
    SptmFermionicSuperposition, SptmFSwapRzRz
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for batch_size in [None, 16]
        for contraction_strategy in contraction_strategy_map.keys()
        for gen_cls in [RandomSptmOperationsGenerator]
    ]
)
def test_global_sptm_det_wo_sptm_f_rxrx(operations_generator: RandomSptmOperationsGenerator, contraction_strategy):
    nif_device = NIFDevice(wires=operations_generator.wires, contraction_strategy=contraction_strategy)
    nif_device.execute_generator(
        operations_generator, n_ops=operations_generator.n_ops, apply=True, reset=True, cache_global_sptm=True
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for operations in [
            ...
            [SptmFSwap(wires=[0, 1]), SptmfRxRx.random(wires=[0, 1]), SptmFSwap(wires=[0, 1])],
            [SptmFSwap(wires=[0, 1]), SptmfRxRx.random(wires=[1, 2]), SptmFSwap(wires=[0, 1])],
            [SptmFSwap(wires=[0, 1]), SptmfRxRx.random(wires=[1, 2]), SptmFSwap(wires=[2, 3])],
            sum([[SptmfRxRx.random(wires=[0, 1]), SptmFSwap(wires=[0, 1])] for _ in range(10)], start=[]),
            sum([[SptmfRxRx.random(wires=[i, i+1]), SptmFSwap(wires=[0, i+1])] for i in range(10)], start=[]),
            sum([[SptmfRxRx.random(wires=[2*i, 2*i+1]), SptmFSwap(wires=[2*i+2, 2*i+3])] for i in range(10)], start=[]),
            # [random.choice([SptmfRxRx, SptmFSwap]).random(wires=w) for w in [[0, 1], [0, 1], [2, 3], [2, 3], [2, 3], [2, 3], [3, 4], [2, 3], [0, 1], [3, 4], [1, 2], [2, 3], [1, 2], [0, 1], [3, 4], [3, 4], [1, 2], [0, 1], [2, 3], [0, 1], [1, 2], [2, 3], [3, 4], [1, 2], [1, 2], [2, 3], [2, 3], [3, 4], [1, 2], [1, 2], [3, 4], [0, 1], [3, 4], [1, 2], [2, 3], [1, 2], [2, 3], [3, 4], [0, 1], [2, 3], [0, 1], [3, 4], [1, 2], [3, 4], [3, 4], [0, 1], [2, 3], [3, 4], [3, 4], [3, 4]]],
        ]
...
def test_global_sptm_det_specific_circuit(operations, contraction_strategy):
    ...
    nif_device.execute_generator(
        iter(operations), n_ops=len(operations), apply=True, reset=True, cache_global_sptm=True
    )
    global_sptm = nif_device.apply_metadata["global_sptm"]
    sptm_det = det(global_sptm)
    np.testing.assert_allclose(
