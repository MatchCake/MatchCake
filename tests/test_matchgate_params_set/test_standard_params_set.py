...
class TestMatchgateStandardParams:
    @classmethod
    def setup_class(cls):
        set_seed(TEST_SEED)
    ...
    def test_standard_params_from_matrix(self, matrix, params):
        params_ = MatchgateStandardParams.from_matrix(matrix)
        assert params_ == params

    def test_standard_params_requires_grad_torch(self):
        batch_size = 2
    ...
    def test_standard_params_from_matrix_requires_grad_torch(self):
        batch_size = 2
    ...
    def test_standard_params_from_vector_requires_grad_torch(self):
        batch_size = 2
        rn_tensor = torch.rand(batch_size, MatchgateStandardParams.N_PARAMS, device="cpu", requires_grad=True)
    ...
    def test_standard_params_from_vector_to_matrix_requires_grad_torch(self):
        batch_size = 2
        rn_tensor = torch.rand(batch_size, MatchgateStandardParams.N_PARAMS, device="cpu", requires_grad=True)
        params = MatchgateStandardParams.from_vector(rn_tensor)
        matrix = params.to_matrix()
    ...
    def test_standard_params_from_matrix_to_vector_requires_grad_torch(self):
        batch_size = 2
        rn_tensor = torch.rand(batch_size, 4, 4, device="cpu", requires_grad=True)
        params = MatchgateStandardParams.from_matrix(rn_tensor)
    ...
    def test_standard_params_from_matrix_grad_torch(self):
        batch_size = 2
        rn_tensor = torch.rand(batch_size, 4, 4, device="cpu", requires_grad=True)
    ...
    def test_standard_params_to_interface(self, interface: Literal["numpy", "torch"]):
        params = MatchgateStandardParams.random()
