...
class TestNeighboursContraction:
    @classmethod
    def setup_class(cls):
        set_seed(TEST_SEED)
    ...
    def test_neighbours_contraction(self, operations, expected_new_operations):
        ...
        assert len(new_operations) == len(expected_new_operations)
    ...
    def test_neighbours_contraction_device_one_line(self, operations):
    ...
    def test_neighbours_contraction_device_one_line_sptm(self, operations):
        nif_device_nh = init_nif_device(wires=2, contraction_method="neighbours")
        nif_device = init_nif_device(wires=2, contraction_method=None)
    ...
    def test_multiples_matchgate_probs_with_qbit_device_nh_contraction(self, params_list, prob_wires):
    ...
    def test_nh_contraction_torch_grad(self, x):
        n_qubits = 4
    ...
    def test_nh_contraction_with_apply_generator(self, circuit_gen, n_wires):
    ...
    def test_nh_contraction_with_apply_generator_sptm_supp(self, n_wires):
    ...
    @pytest.mark.parametrize("n_wires", [n for n in range(2, 12)])
    def test_nh_contraction_with_apply_generator_supp(self, n_wires):
        wires = np.arange(n_wires)
