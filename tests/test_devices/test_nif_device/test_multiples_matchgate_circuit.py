...
def multiples_matchgate_circuit(params_list, initial_state=None, **kwargs):
    ...
    for params in params_list:
        mg_params = mgp.MatchgatePolarParams.parse_from_any(params)
...
@pytest.mark.parametrize(
    "params_list,n_wires",
    [([MatchgateOperation.random_params(seed=0), MatchgateOperation.random_params(seed=1)], 4)]
    + [
        (
            [mgp.MatchgatePolarParams(r0=1, r1=1) for _ in range(num_gates)],
            num_wires,
        )
    ...
    + [
        (
            [MatchgateOperation.random_params(seed=i * j + j) for i in range(num_gates)],
            num_wires,
        )
        for j in range(3)
        for num_wires in range(2, 6)
...
class TestNIFDeviceProbabilities:
    ...
    def test_multiples_matchgate_probs_with_qubit_device_op_gen_sptm_unitary(
        ...
        def ground_truth_circuit():
            state_prep_op.queue()
            nif_device.global_sptm.to_qubit_unitary().queue()
