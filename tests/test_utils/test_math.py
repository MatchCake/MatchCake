...
@pytest.mark.parametrize(
    ...
    [
        ...
        for n_terms in range(
            LOW_PRECISION_TERMS, LOW_PRECISION_TERMS + N_RANDOM_TESTS_PER_CASE
        )
    ],
)
def test_exp_taylor_series_approx(x, n_terms):
    ...
    np.testing.assert_allclose(
        out, target, atol=ATOL_APPROX_COMPARISON, rtol=RTOL_APPROX_COMPARISON
    )
...
@pytest.mark.parametrize(
    ...
    [
        (np.random.uniform(-HIGH_VALUE, HIGH_VALUE), n_terms)
        for n_terms in range(
            MEDIUM_PRECISION_TERMS, MEDIUM_PRECISION_TERMS + N_RANDOM_TESTS_PER_CASE
        )
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (np.random.uniform(-HIGH_VALUE, HIGH_VALUE), n_terms)
        for n_terms in range(
            HIGH_PRECISION_TERMS, HIGH_PRECISION_TERMS + N_RANDOM_TESTS_PER_CASE
        )
    ],
)
def test_exp_taylor_series_high_precision(x, n_terms):
    ...
    out = utils.math.exp_taylor_series(x, n_terms)
    np.testing.assert_allclose(
        out, target, atol=ATOL_SCALAR_COMPARISON, rtol=RTOL_SCALAR_COMPARISON
    )
...
def test_random_index(probs_shape):
    ...
    counts = np.apply_along_axis(
        lambda x: np.bincount(x, minlength=probs.shape[-1]), axis=0, arr=indexes
    ).T
...
def test_random_index_3d_none(probs_shape):
    ...
    probs_normed = probs / probs.sum(axis=-1, keepdims=True)
    indexes = np.stack(
        [utils.math.random_index(probs_normed, n=None) for _ in range(n)], axis=0
    )
    counts = np.apply_along_axis(
