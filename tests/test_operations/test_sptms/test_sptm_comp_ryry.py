...
class TestSptmRyRy:
    ...
    def test_sptm_is_so4(self, theta, phi):
        params = np.asarray([theta, phi]).reshape(-1, 2).squeeze()
        sptm_obj = SptmCompRyRy(params, wires=[0, 1])
        assert sptm_obj.check_is_in_so4()

    @pytest.mark.parametrize(
        "theta",
        [np.full(batch_size, theta) for batch_size in [1, 4] for theta in SptmCompRyRy.EQUAL_ALLOWED_ANGLES],
    )
    ...
    def test_sptm_is_so4_out_of_angles(self, theta, phi):
        params = np.asarray([theta, phi]).reshape(-1, 2).squeeze()
        sptm_obj = SptmCompRyRy(params, wires=[0, 1], check_angles=False, clip_angles=True)
        assert sptm_obj.check_is_in_so4()
    ...
    def test_matchgate_equal_to_sptm_adjoint(self, theta, phi):
        params = np.asarray([theta, phi]).reshape(-1, 2).squeeze()
        params = SptmCompRyRy.clip_angles(params)
        matchgate = CompRyRy(params, wires=[0, 1]).adjoint()
        m_sptm = matchgate.single_particle_transition_matrix
        sptm = SptmCompRyRy(params, wires=[0, 1]).adjoint().matrix()
        np.testing.assert_allclose(
            sptm,
            m_sptm,
            atol=ATOL_APPROX_COMPARISON,
