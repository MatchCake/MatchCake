...
from matchcake.matchgate_parameter_sets.transfer_functions import (
    ...
    MatchgateParams, MatchgatePolarParams, MatchgateStandardHamiltonianParams,
    MatchgateStandardParams, _transfer_funcs_by_type, params_to)
...
@pytest.mark.parametrize(
    "__from_params",
    [__cls.random() for __cls in _transfer_funcs_by_type.keys() for _ in range(N_RANDOM_TESTS_PER_CASE)],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for _to_cls in ([MatchgatePolarParams] if not MatchgatePolarParams.ALLOW_COMPLEX_PARAMS else [])
...
@pytest.mark.parametrize(
    "__from_params",
    [MatchgateComposedHamiltonianParams.random() for _ in range(N_RANDOM_TESTS_PER_CASE)],
)
...
@pytest.mark.parametrize(
    "__from_params",
    [MatchgateStandardHamiltonianParams.random() for _ in range(N_RANDOM_TESTS_PER_CASE)],
)
