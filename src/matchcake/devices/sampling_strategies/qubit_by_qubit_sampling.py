...
class QubitByQubitSampling(KQubitsByKQubitsSampling):
    ...
    def generate_samples(
        ...
        **kwargs,
        ...
        probs = to_numpy(
            qml.math.stack([state_prob_func([i], Wires([0])) for i in [0, 1]], axis=-1)
        )
        ...
        for j in range(1, device.num_wires):
            ...
            zeros_states = qml.math.concatenate(
                [unique_states, qml.math.full((*unique_batch_shape, 1), 0)], -1
            )
            ones_states = qml.math.concatenate(
                [unique_states, qml.math.full((*unique_batch_shape, 1), 1)], -1
            )
            all_states = (
                qml.math.stack([zeros_states, ones_states], axis=0)
                .reshape(-1, j + 1)
                .astype(int)
            )
            ...
            for i, state in enumerate(all_states):
                ...
                probs[..., state[-1]] = np.where(
                    mask, all_probs[i], probs[..., state[-1]]
                )
