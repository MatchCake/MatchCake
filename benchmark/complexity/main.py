...
def main(kwargs):
    ...
    n_gates = int(kwargs.get("n_gates", 10 * np.max(n_wires)))
    # n_gates = 1
    # n_gates = "linear"
    # n_wires = np.arange(2, 1024, 100)
    ...
    benchmark_pipeline = BenchmarkPipeline.from_pickle_or_new(
        ...
        # n_pts=kwargs.get("n_pts", 10),
        n_variance_pts=kwargs.get("n_variance_pts", 10),
        n_wires=n_wires,
        n_gates=n_gates,
